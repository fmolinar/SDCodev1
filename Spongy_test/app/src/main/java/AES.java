...
class AES {
    ...
    public String aesRun(String data, int mode) {
        ...
        for (int p = 0; p < nParts; p++) {
            ...
            if (mode == 1) {                    //mac
                ...
                for (int r = 0; r < numRounds; r++) {
                    if (Roundone == 0) {
                        for (int k = 0; k < partByte.length; k++)
                            input[k] = (byte) (partByte[k] ^ iv[k]);
                        cipher = aes.encrypt(input);
                    } else {
                        for (int k = 0; k < partByte.length; k++)
                            input[k] = (byte) (partByte[k] ^ cipher[k]);
                        cipher = aes.encrypt(input);
        ...
        mac = Util.byteArrayToString(res1);
    ...
    public String Encrypt(String data) {
        ...
        return aesRun(data, 1);
    }

    public String Decrypt(String data) {
        return aesRun(data, 2).trim();
    }
    ...
    public static void main (String [] args) throws Exception {
        ...
        //User A
        aes.setKey(key);  // choose password
